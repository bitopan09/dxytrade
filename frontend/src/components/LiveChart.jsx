...
const LiveChart = () => {
    ...
    useEffect(() => {
        ...
        const fetchInitialData = async () => {
            try {
                const response = await fetch(`${API_BASE_URL}/candles?limit=50`);
                if (response.ok) {
                    const data = await response.json();
                    setPriceData(
                        data.map(item => ({
                            time: new Date(item.timestamp),
                            price: item.close || item.price || 104.500
                        }))
                    );
                }
            } catch (error) {
            ...
            <ResponsiveContainer width="100%" height={300}>
                <LineChart data={chartData}>
                    ...
                    <YAxis 
                        ...
                        domain={['dataMin - 0.00050', 'dataMax + 0.00050']} 
                        tickFormatter={(val) => val.toFixed(5)}
                    />
                    <Tooltip
                        contentStyle={{ background: 'rgba(15,23,42,0.95)', border: '1px solid rgba(99,102,241,0.3)', borderRadius: '10px', color: '#f1f5f9', fontFamily: 'JetBrains Mono, monospace', fontSize: '13px' }}
                        labelStyle={{ color: '#94a3b8' }}
                    />
                    <Line type="monotone" dataKey="price" stroke="#06b6d4" strokeWidth={2} dot={false} activeDot={{ r: 5, fill: '#06b6d4', stroke: '#0a0e1a', strokeWidth: 2 }} />
                </LineChart>
            </ResponsiveContainer>
            <div className="chart-info">
                <p>Current Price: ${priceData.length > 0 ? priceData[priceData.length - 1].price.toFixed(5) : 'Loading...'}</p>
