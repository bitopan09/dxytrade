...
const ActiveTrades = () => {
    ...
    const calculatePnL = (trade) => {
        ...
        if (trade.action === 'BUY') {
            return ((currentPrice - trade.entry_price) * trade.quantity * 100000) / currentPrice;
        } else {
            return ((trade.entry_price - currentPrice) * trade.quantity * 100000) / currentPrice;
        }
    };
                        ...
                        return (
                            <div key={trade.id} className={`active-trade-card ${trade.action.toLowerCase()}`}>
                                ...
                                <div className="trade-details">
                                    <p><strong>Entry:</strong> ${trade.entry_price?.toFixed(5)}</p>
                                    <p><strong>Live Price:</strong> ${currentPrice.toFixed(5)}</p>
                                    <p><strong>SL:</strong> ${trade.stop_loss !== undefined ? trade.stop_loss.toFixed(5) : (trade.sl?.toFixed(5) || 'N/A')}</p>
                                    <p><strong>TP2:</strong> ${trade.take_profit_2 !== undefined ? trade.take_profit_2.toFixed(5) : (trade.tp2?.toFixed(5) || 'N/A')}</p>
                                </div>
                                <button 
                                    className="btn-exit" 
